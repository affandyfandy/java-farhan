...
class CustomerControllerTest {
    ...
    @Test
    void shouldGetAllCustomer_withCustomers() throws Exception {
        // Prepare test data
    ...
    @Test
    void shouldGetAllCustomer_withNoCustomers() throws Exception {
        // Prepare test data
        Pageable pageable = PageRequest.of(0, 20);
    ...
    @Test
    void shouldCreateCustomer_withValidFormat() throws Exception {
    ...
    @Test
    void shouldCreateCustomer_withInvalidFormat() throws Exception {
    ...
    @Test
    void shouldUpdateCustomer_withCustomerNoExist() throws Exception {
        UUID customerId = UUID.randomUUID();
    ...
    @Test
    void shouldUpdateCustomerToStatusActive() throws Exception {
        UUID customerId = UUID.randomUUID();
        CustomerDTO customerDTO = new CustomerDTO(customerId, "Updated Customer", "+62123456789", Status.ACTIVE);
    ...
    @Test
    void shouldUpdateCustomerToStatusDeactive() throws Exception {
        UUID customerId = UUID.randomUUID();
        CustomerDTO customerDTO = new CustomerDTO(customerId, "Updated Customer", "+62123456789", Status.DEACTIVATED);

        when(customerService.updateCustomerStatus(any(UUID.class), any(Status.class))).thenReturn(customerDTO);

        mockMvc.perform(put("/api/v1/customers/deactive/" + customerId))
                .andExpect(status().isOk())
                .andExpect(content().json("{\"id\": \"" + customerDTO.getId() + "\", \"name\": \"Updated Customer\", \"phoneNumber\": \"+62123456789\", \"status\": \"DEACTIVATED\"}"));
    }

    @Test
    void shouldUpdateCustomerToStatusDeactive_withCustomerNotExist() throws Exception {
        UUID customerId = UUID.randomUUID();
