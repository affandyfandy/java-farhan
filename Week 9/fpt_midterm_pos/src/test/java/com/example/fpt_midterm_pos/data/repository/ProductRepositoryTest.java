...
class ProductRepositoryTest {
    ...
    @Test
    void shouldFindProductByNameContainingAndStatus() {
    ...
    @Test
    void shouldFindProductByFilters() {
        Page<Product> products = productRepository.findByFilters(Status.ACTIVE, "Test", 50.0, 150.0, PageRequest.of(0, 10));
        assertThat(products.getTotalElements()).isPositive();
    }

    @Test
    void shouldFindByNameContainingEmptyResult() {
        assertThat(productRepository.findByNameContainingAndStatus("NonExistent", Status.ACTIVE)).isEmpty();
    }

    @Test
    void shouldFindAllByStatusEmptyResult() {
        assertThat(productRepository.findAllByStatus(Status.DEACTIVATED, Pageable.unpaged())).isEmpty();
    }
